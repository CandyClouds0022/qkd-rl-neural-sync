...
class ActionDistributionLogger(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_freq == 0:
            ...
            #(distribution non squashata: prendiamo direttamente mean/std dalla policy)
...
def train_agent(total_tmstpd):
    ...
    env=QKDEnv()
    eval_env=QKDEnv() #evaluation environment, to be used in the EvalCallback, to evaluate the model during training
    ...
    model=PPO('MlpPolicy', env, verbose=1, learning_rate=0.003, n_steps=2048, batch_size=64, ent_coef=0.01)#, n_epochs=10)
    ...
    action_logger=ActionDistributionLogger(check_freq=2000) #DIAGNOSTIC related (statistics for the ation distribution)
    ...
    eval_callback=EvalCallback(eval_env=eval_env, best_model_save_path=best_model_dir, log_path=eval_log_dir, eval_freq=5000, deterministic=True, render=False)
    ...
    with open(report_path, "a") as f: #'a'ppend mode
        json.dump(report, f)
        f.write("\n")
